"""Plot separate figures for the all-method baseline experiment."""
...
RESULTS_DIR = PROJECT_ROOT / 'results' / 'experiments' / 'baseline_all_methods__mcar20__seed42'
...
def main() -> None:
    _style()
    detailed = pd.read_csv(TABLES_DIR / '01_baseline_all_methods_detailed_results.csv')
    summary = pd.read_csv(TABLES_DIR / '02_baseline_all_methods_summary.csv')
    ...
    ax.set_title('Cluster Structure Preservation by Dataset', loc='left', fontsize=20)
    ...
    heatmap_path = _save(fig, '01_cluster_structure_heatmap.png')
    ...
    ax.set_title('Overall Baseline Score Ranking', loc='left', fontsize=20)
    ...
    score_path = _save(fig, '02_overall_score_bar.png')
    ...
    offsets = {
        'median': (8, 10), 'mice': (8, -14), 'em': (8, 10), 'knni': (8, 12), 'mean': (8, -16),
        'iterative_xgboost': (8, 8), 'missforest': (8, 10), 'gain': (8, -16), 'miwae': (8, 10),
        'hivae': (8, -16), 'nomi': (8, 10), 'grape': (8, 10), 'diffputer': (8, -18),
    }
    for _, row in scatter.iterrows():
        dx, dy = offsets.get(row['method'], (6, 6))
        ax.annotate(
            row['method'],
            (row['mean_runtime_sec'], row['mean_masked_nrmse']),
            xytext=(dx, dy),
            textcoords='offset points',
            fontsize=9,
            bbox={'boxstyle': 'round,pad=0.18', 'fc': '#ffffff', 'ec': 'none', 'alpha': 0.9},
        )
    ax.set_title('Speed vs Normalized Reconstruction Error', loc='left', fontsize=20)
    ...
    scatter_path = _save(fig, '03_speed_vs_error_scatter.png')
    ...
    ax.set_title('Top Methods on Key Quality Metrics', loc='left', fontsize=20)
    ax.set_xlabel('Metric value', labelpad=12)
    ax.set_ylabel('')
    ax.tick_params(axis='y', labelsize=11)
    ax.legend(title='Metric', frameon=False, loc='lower right', fontsize=9, title_fontsize=10)
    profile_path = _save(fig, '04_top_quality_metrics_bar.png')
    ...
    rank_path = _save(fig, '05_method_ranking_curve.png')
